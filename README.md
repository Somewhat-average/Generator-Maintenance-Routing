...
## Managing Clients
Run `python manage_clients.py` for a terminal menu to add, edit, or remove clients - no need to edit `generator_clients.csv` by hand. (The GUI's "Manage Clients" tab covers the same ground if you'd rather not use a terminal.)
...
- **The routing matrices stay in sync.** Every add, edit, or removal automatically rebuilds `distance_matrix.csv` and `duration_matrix.csv`, so route generation always has up-to-date data. (If that step fails, e.g. no internet connection, run `get_matrix.py` manually once you're back online.)
